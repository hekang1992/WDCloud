...
extension SearchJudicialAuctioniewViewController: HGSegmentedPageViewControllerDelegate {
    ...
    func readHistoryUI(data: [rowsModel]) {
        for (index, model) in data.enumerated() {
            ...
            listView.block = { [weak self] in
                ...
                if type == "1" {//企业
                    let pageUrl = "\(base_url)/litigation-risk/judicial-action"
