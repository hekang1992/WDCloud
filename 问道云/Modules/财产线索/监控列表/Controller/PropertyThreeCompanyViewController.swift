...
class PropertyThreeCompanyViewController: WDBaseViewController {
    ...
    override func viewDidLoad() {
        ...
        let threeMenu = MenuAction(title: "更多", style: .typeCustom)!
        threeMenu.displayCustomWithMenu = { [weak self] in
            guard let self = self else { return UIView() }
            moreView.sureBlock = { [weak self] btn, index in
                self?.pageNum = 1
                if index == 0 {
                    self?.registerCapital = ""
                }else {
                    self?.registerCapital = String(index)
                }
                let title = btn.titleLabel?.text ?? ""
                if title == "全部" {
                    threeMenu.adjustTitle("更多", textColor: UIColor.init(cssStr: "#666666"))
                }else {
                    threeMenu.adjustTitle(title, textColor: UIColor.init(cssStr: "#547AFF"))
                }
                self?.getListInfo()
            }
            return moreView
...
extension PropertyThreeCompanyViewController: UITextFieldDelegate {
...
//extension PropertyThreeCompanyViewController: SkeletonTableViewDataSource {
...
extension PropertyThreeCompanyViewController: UITableViewDelegate, UITableViewDataSource {
