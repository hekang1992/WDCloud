//
//  AddPropertyPeopleUnioViewController.swift
//  问道云
//
...
class AddPropertyPeopleUnioViewController: WDBaseViewController {
    ...
    override func viewDidLoad() {
        ...
        self.searchTx.text = model?.legalName ?? ""
        getMessageInfo()
    }
    
}

extension AddPropertyPeopleUnioViewController {
    
    private func getMessageInfo() {
        let dict = ["connectType": 1]
    ...
    private func addInfo() {
        let dict = ["relationId": entityId,
                    ...
                    "relationType": 1,
                    "beRelationId": model?.legalInfo?.first?.legal_id ?? 0,
                    "connectId": connectId,
                    "beRelationName": self.searchTx.text ?? "",
                    "beRelationType": "2"] as [String : Any]
        let man = RequestManager()
        man.requestAPI(params: dict,
