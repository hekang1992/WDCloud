...
class SearchDondDefaultViewController: WDBaseViewController {
    ...
    lazy var headView: PropertyHeadView = {
        ...
        headView.headView.titlelabel.text = "债券违约"
        return headView
    }()
...
extension SearchDondDefaultViewController: UITextFieldDelegate {
...
extension SearchDondDefaultViewController: JXPagingViewDelegate, JXSegmentedViewDelegate {
...
/** 网络数据请求 */
extension SearchDondDefaultViewController {
    ...
    private func getNumInfo(from keywords: String){
        let dict = ["keyword": keywords, "riskType": "BOND_DEFAULT_COUNT"]
        man.requestAPI(params: dict,
    ...
    private func getHotsSearchInfo() {
        ...
        group.enter()
        getLastSearchInfo(from: "21") { [weak self] modelArray in
        ...
        group.enter()
        getLastHistroyInfo(from: "21") { [weak self] modelArray in
            self?.historyArray = modelArray
            group.leave()
        }
        
        group.enter()
        getLastHotsInfo(from: "21") { [weak self] modelArray in
