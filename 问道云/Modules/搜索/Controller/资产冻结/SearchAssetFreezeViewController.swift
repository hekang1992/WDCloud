//
//  SearchAssetFreezeViewController.swift
//  问道云
//
...
class SearchAssetFreezeViewController: WDBaseViewController {
    ...
    lazy var headView: PropertyHeadView = {
        ...
        headView.headView.titlelabel.text = "资产冻结"
        return headView
    }()
    ...
    lazy var companyVc: SearchCompanyAssetFreezeViewController = {
        let companyVc = SearchCompanyAssetFreezeViewController()
        return companyVc
    }()
    
    lazy var peopleVc: SearchPeopleAssetFreezeViewController = {
        let peopleVc = SearchPeopleAssetFreezeViewController()
        return peopleVc
    }()
...
extension SearchAssetFreezeViewController: UITextFieldDelegate {
...
extension SearchAssetFreezeViewController: JXPagingViewDelegate, JXSegmentedViewDelegate {
...
/** 网络数据请求 */
extension SearchAssetFreezeViewController {
    ...
    private func getNumInfo(from keywords: String){
        let dict = ["keyword": keywords, "riskType": "ASSET_FREEZE_COUNT"]
        man.requestAPI(params: dict,
    ...
    private func getHotsSearchInfo() {
        ...
        group.enter()
        getLastSearchInfo(from: "22") { [weak self] modelArray in
        ...
        group.enter()
        getLastHistroyInfo(from: "22") { [weak self] modelArray in
            self?.historyArray = modelArray
            group.leave()
        }
        
        group.enter()
        getLastHotsInfo(from: "22") { [weak self] modelArray in
