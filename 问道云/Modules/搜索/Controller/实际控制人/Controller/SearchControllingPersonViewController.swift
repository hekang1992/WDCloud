//
//  SearchControllingPersonViewController.swift
//  问道云
//
...
extension SearchControllingPersonViewController: UITextFieldDelegate {
...
extension SearchControllingPersonViewController {
    ...
    private func getHotsSearchInfo() {
        ...
        ViewHud.addLoadView()
        group.enter()
        getLastSearchInfo(from: "40") { [weak self] modelArray in
        ...
        group.enter()
        getLastHistroyInfo(from: "40") { [weak self] modelArray in
            self?.historyArray = modelArray
            group.leave()
        }
        
        group.enter()
        getLastHotsInfo(from: "40") { [weak self] modelArray in
