//
//  SearchLoanDefaultViewController.swift
//  问道云
//
...
class SearchLoanDefaultViewController: WDBaseViewController {
    ...
    lazy var headView: PropertyHeadView = {
        ...
        headView.headView.titlelabel.text = "贷款逾期"
        return headView
    }()
    ...
    lazy var companyVc: SearchCompanyLoanDefaultViewController = {
        let companyVc = SearchCompanyLoanDefaultViewController()
        return companyVc
    }()
    
    lazy var peopleVc: SearchPeopleLoanDefaultViewController = {
        let peopleVc = SearchPeopleLoanDefaultViewController()
        return peopleVc
    }()
...
extension SearchLoanDefaultViewController: UITextFieldDelegate {
...
extension SearchLoanDefaultViewController: JXPagingViewDelegate, JXSegmentedViewDelegate {
...
/** 网络数据请求 */
extension SearchLoanDefaultViewController {
    ...
    private func getNumInfo(from keywords: String){
        let dict = ["keyword": keywords, "riskType": "LOAN_OVERDUE_COUNT"]
        man.requestAPI(params: dict,
    ...
    private func getHotsSearchInfo() {
        ...
        group.enter()
        getLastSearchInfo(from: "20") { [weak self] modelArray in
        ...
        group.enter()
        getLastHistroyInfo(from: "20") { [weak self] modelArray in
            self?.historyArray = modelArray
            group.leave()
        }
        
        group.enter()
        getLastHotsInfo(from: "20") { [weak self] modelArray in
