//
//  SearchEnvironmentalPenaltyViewController.swift
//  问道云
//
...
class SearchEnvironmentalPenaltyViewController: WDBaseViewController {
    ...
    lazy var headView: PropertyHeadView = {
        ...
        headView.headView.titlelabel.text = "环保处罚"
        return headView
    }()
...
extension SearchEnvironmentalPenaltyViewController: UITextFieldDelegate {
...
extension SearchEnvironmentalPenaltyViewController: JXPagingViewDelegate, JXSegmentedViewDelegate {
...
extension SearchEnvironmentalPenaltyViewController {
    ...
    private func getNumInfo(from keywords: String){
        let dict = ["keyword": keywords, "riskType": "ENV_PENALTIES_COUNT"]
        man.requestAPI(params: dict,
    ...
    private func getHotsSearchInfo() {
        ...
        group.enter()
        getLastSearchInfo(from: "18") { [weak self] modelArray in
        ...
        group.enter()
        getLastHistroyInfo(from: "18") { [weak self] modelArray in
            self?.historyArray = modelArray
            group.leave()
        }
        
        group.enter()
        getLastHotsInfo(from: "18") { [weak self] modelArray in
