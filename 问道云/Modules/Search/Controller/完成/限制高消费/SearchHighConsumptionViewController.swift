...
class SearchHighConsumptionViewController: WDBaseViewController {
    ...
    lazy var headView: PropertyHeadView = {
        ...
        headView.headView.titlelabel.text = "限制高消费"
        return headView
    }()
...
extension SearchHighConsumptionViewController: JXPagingViewDelegate, JXSegmentedViewDelegate {
...
/** 网络数据请求 */
extension SearchHighConsumptionViewController {
    ...
    private func getNumInfo(from keywords: String){
        let dict = ["keyword": keywords, "riskType": "RHC_COUNT"]
        man.requestAPI(params: dict,
    ...
    private func getHotsSearchInfo() {
        ...
        group.enter()
        getLastSearchInfo(from: "13") { [weak self] modelArray in
        ...
        group.enter()
        getLastHistroyInfo(from: "13") { [weak self] modelArray in
            self?.historyArray = modelArray
            group.leave()
        }
        
        group.enter()
        getLastHotsInfo(from: "13") { [weak self] modelArray in
