//
//  SearchExternalGuaranteeViewController.swift
//  问道云
//
...
class SearchExternalGuaranteeViewController: WDBaseViewController {
    ...
    lazy var headView: PropertyHeadView = {
        ...
        headView.headView.titlelabel.text = "对外担保"
        return headView
    }()
    ...
    lazy var companyVc: SearchCompanyExternalGuaranteeViewController = {
        let companyVc = SearchCompanyExternalGuaranteeViewController()
        return companyVc
    }()
    
    lazy var peopleVc: SearchPeopleExternalGuaranteeViewController = {
        let peopleVc = SearchPeopleExternalGuaranteeViewController()
        return peopleVc
    }()
...
extension SearchExternalGuaranteeViewController: JXPagingViewDelegate, JXSegmentedViewDelegate {
...
/** 网络数据请求 */
extension SearchExternalGuaranteeViewController {
    ...
    private func getNumInfo(from keywords: String){
        let dict = ["keyword": keywords, "riskType": "EXT_GUARANTEE_COUNT"]
        man.requestAPI(params: dict,
    ...
    private func getHotsSearchInfo() {
        ...
        group.enter()
        getLastSearchInfo(from: "25") { [weak self] modelArray in
        ...
        group.enter()
        getLastHistroyInfo(from: "25") { [weak self] modelArray in
            self?.historyArray = modelArray
            group.leave()
        }
        
        group.enter()
        getLastHotsInfo(from: "25") { [weak self] modelArray in
