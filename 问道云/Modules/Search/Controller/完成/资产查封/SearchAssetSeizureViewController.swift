//
//  SearchAssetSeizureViewController.swift
//  问道云
//
...
class SearchAssetSeizureViewController: WDBaseViewController {
    ...
    lazy var headView: PropertyHeadView = {
        ...
        headView.headView.titlelabel.text = "资产查封"
        return headView
    }()
    ...
    lazy var companyVc: SearchCompanyAssetSeizureViewController = {
        let companyVc = SearchCompanyAssetSeizureViewController()
        return companyVc
    }()
    
    lazy var peopleVc: SearchPeopleAssetSeizureViewController = {
        let peopleVc = SearchPeopleAssetSeizureViewController()
        return peopleVc
    }()
...
extension SearchAssetSeizureViewController: UITextFieldDelegate {
...
extension SearchAssetSeizureViewController: JXPagingViewDelegate, JXSegmentedViewDelegate {
...
/** 网络数据请求 */
extension SearchAssetSeizureViewController {
    ...
    private func getNumInfo(from keywords: String){
        let dict = ["keyword": keywords, "riskType": "ASSET_SEIZE_COUNT"]
        man.requestAPI(params: dict,
    ...
    private func getHotsSearchInfo() {
        ...
        group.enter()
        getLastSearchInfo(from: "23") { [weak self] modelArray in
        ...
        group.enter()
        getLastHistroyInfo(from: "23") { [weak self] modelArray in
            self?.historyArray = modelArray
            group.leave()
        }
        
        group.enter()
        getLastHotsInfo(from: "23") { [weak self] modelArray in
