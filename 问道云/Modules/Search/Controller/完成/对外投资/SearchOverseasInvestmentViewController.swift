//
//  SearchOverseasInvestmentViewController.swift
//  问道云
//
...
class SearchOverseasInvestmentViewController: WDBaseViewController {
    ...
    lazy var headView: PropertyHeadView = {
        ...
        headView.headView.titlelabel.text = "对外投资"
        return headView
    }()
    ...
    lazy var companyVc: SearchCompanyOverseasInvestmentViewController = {
        let companyVc = SearchCompanyOverseasInvestmentViewController()
        return companyVc
    }()
    
    lazy var peopleVc: SearchPeopleOverseasInvestmentViewController = {
        let peopleVc = SearchPeopleOverseasInvestmentViewController()
        return peopleVc
    }()
...
extension SearchOverseasInvestmentViewController: JXPagingViewDelegate, JXSegmentedViewDelegate {
...
/** 网络数据请求 */
extension SearchOverseasInvestmentViewController {
    ...
    private func getHotsSearchInfo() {
        ...
        group.enter()
        getLastSearchInfo(from: "26") { [weak self] modelArray in
        ...
        group.enter()
        getLastHistroyInfo(from: "26") { [weak self] modelArray in
            self?.historyArray = modelArray
            group.leave()
        }
        
        group.enter()
        getLastHotsInfo(from: "26") { [weak self] modelArray in
