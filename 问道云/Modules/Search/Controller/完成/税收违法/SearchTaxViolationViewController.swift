//
//  SearchTaxViolationViewController.swift
//  问道云
//
...
class SearchTaxViolationViewController: WDBaseViewController {
    ...
    lazy var headView: PropertyHeadView = {
        ...
        headView.headView.titlelabel.text = "税收违法"
        return headView
    }()
    ...
    lazy var companyVc: SearchCompanyTaxViolationViewController = {
        let companyVc = SearchCompanyTaxViolationViewController()
        return companyVc
    }()
    
    lazy var peopleVc: SearchPeopleTaxViolationViewController = {
        let peopleVc = SearchPeopleTaxViolationViewController()
        return peopleVc
    }()
...
extension SearchTaxViolationViewController: UITextFieldDelegate {
...
extension SearchTaxViolationViewController: JXPagingViewDelegate, JXSegmentedViewDelegate {
...
/** 网络数据请求 */
extension SearchTaxViolationViewController {
    ...
    private func getNumInfo(from keywords: String){
        let dict = ["keyword": keywords, "riskType": "TAX_VIOLATION_COUNT"]
        man.requestAPI(params: dict,
    ...
    private func getHotsSearchInfo() {
        ...
        group.enter()
        getLastSearchInfo(from: "17") { [weak self] modelArray in
        ...
        group.enter()
        getLastHistroyInfo(from: "17") { [weak self] modelArray in
            self?.historyArray = modelArray
            group.leave()
        }
        
        group.enter()
        getLastHotsInfo(from: "17") { [weak self] modelArray in
