...
#warning 我自己新增的
        if (_oneListDataSource.count < 7) {
            ...
            tableViewFrame.size.height = totalHeight + 2;
...
- (void)resetActionTitle:(ItemModel *)selectModel selectRow:(NSInteger)row {
    ...
    if(row == 0) {
        ...
        [self.action setTitleColor:[UIColor colorWithRed:0.4 green:0.4 blue:0.4 alpha:1] forState:UIControlStateNormal];
    }else {
        [self.action setTitle:selectModel.displayText forState:UIControlStateNormal];
        [self.action setTitleColor:[UIColor colorWithRed:0.33 green:0.48 blue:1 alpha:1] forState:UIControlStateNormal];
    }
